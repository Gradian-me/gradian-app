...
import { Check, X, Clock, Play, Loader2, Eye, Timer, AlertCircle, GripVertical, Edit2, Trash2 } from 'lucide-react';
...
export const TodoList: React.FC<TodoListProps> = ({
  ...
  const getStatusIcon = (todo: Todo) => {
    ...
    return (
      <div className="flex-shrink-0 w-6 h-6 rounded-full border-2 border-gray-300 dark:border-gray-600 flex items-center justify-center">
        <Clock className="w-3 h-3 text-gray-400" />
      </div>
    );
  };
      ...
      >
        ...
        >
          <AccordionItem value="execution-plan" className="border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800">
            <AccordionTrigger className="px-4 py-3 hover:no-underline bg-gray-50 dark:bg-gray-900/50 rounded-t-lg">
              <div className="flex items-center justify-between w-full pr-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-600 dark:text-gray-400 mt-0.5 text-left">
    ...
    >
      ...
      {!isCompleted && todo.status !== 'in_progress' && (
        <button
          {...attributes}
          {...listeners}
          className="cursor-grab active:cursor-grabbing text-gray-400 hover:text-gray-600 transition-colors p-0.5 shrink-0 dark:text-gray-500 dark:hover:text-gray-400 mt-1"
        >
