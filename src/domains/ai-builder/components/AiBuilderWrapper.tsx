...
export function AiBuilderWrapper({
  ...
  const [showDoMagicAgainConfirm, setShowDoMagicAgainConfirm] = useState(false);
  ...
  const doGenerate = useCallback(() => {
    ...
    if (selectedAgent && Object.keys(formValues).length > 0) {
      ...
      body = { ...formBody, ...body };
      extra_body = { ...formExtraBody, ...extra_body };
    }
  ...
  const handleGenerate = useCallback(() => {
    if (!selectedAgentId) return;
    // If there is already a response, confirm that previous result will be lost before resetting and re-running
    const hasExistingResponse = Boolean(aiResponse && String(aiResponse).trim());
    if (hasExistingResponse) {
      setShowDoMagicAgainConfirm(true);
      return;
    }
    doGenerate();
  }, [selectedAgentId, aiResponse, doGenerate]);

  const handleDoMagicAgainConfirm = useCallback(() => {
    setShowDoMagicAgainConfirm(false);
    clearResponse();
    doGenerate();
  }, [clearResponse, doGenerate]);
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      {showResetButton && (
        <ConfirmationMessage
          ...
          buttons={[
            ...
            {
              label: getT(TRANSLATION_KEYS.BUTTON_RESET, language, defaultLang),
              variant: 'destructive',
              icon: 'RotateCcw',
      ...
      {/* Do the Magic again – confirm previous result will be lost, then reset and run */}
      <ConfirmationMessage
        isOpen={showDoMagicAgainConfirm}
        onOpenChange={setShowDoMagicAgainConfirm}
