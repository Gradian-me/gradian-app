...
import {
  AllowedCharConfig,
  buildAllowedCharConfig,
  buildAllowedRegex,
  buildHelperTextFromPattern,
  getPatternTemplate,
  sanitizeByAllowedConfig,
} from '@/gradian-ui/shared/utils/text-utils';
...
export const NameInput = forwardRef<FormElementRef, NameInputProps>(
  (
    ...
    const patternTemplate = useMemo(
      () => getPatternTemplate(config?.validation?.pattern),
      [config?.validation?.pattern]
    );
    const allowedCharConfig = useMemo(
      () => buildAllowedCharConfig(patternTemplate),
      [patternTemplate]
    );
    ...
    useImperativeHandle(ref, () => ({
      ...
      validate: () => {
        ...
        if (!config?.validation) {
          if (!required && !strValue) return true;
          return allowedPattern.test(strValue);
        }
        const result = validateField(value, config.validation);
        const isTemplatePattern = !!patternTemplate;
        const passesAllowedPattern = !strValue || allowedPattern.test(strValue);
        // When using a symbolic pattern (e.g. "Aa_-@"), enforce allowed character set in addition to validateField
        return result.isValid && (isTemplatePattern ? passesAllowedPattern : true);
      },
      reset: () => onChange?.(''),
      getValue: () => value,
      setValue: (newValue) =>
    ...
    const isValidName = useMemo(
      () => Boolean(value) && allowedPattern.test(String(value ?? '')),
      [value, allowedPattern]
    );

    const dynamicHelperMessage = useMemo(
      () => buildHelperTextFromPattern(patternTemplate, forbiddenSymbolsMessage),
      [patternTemplate, forbiddenSymbolsMessage]
    );
