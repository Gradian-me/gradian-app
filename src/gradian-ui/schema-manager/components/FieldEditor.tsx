...
import { toKebabCase, buildAllowedCharConfig, sanitizeByAllowedConfig } from '@/gradian-ui/shared/utils/text-utils';
...
export function FieldEditor({
  ...
  const fieldNameCharConfig = buildAllowedCharConfig('a-');

  const normalizeFieldName = (label: string): string => {
    const base = toKebabCase(label || '');
    return sanitizeByAllowedConfig(base, fieldNameCharConfig, {
  ...
  const handleLabelChange = (value: string) => {
    ...
    if (!nameManuallyEdited) {
      const autoGeneratedName = normalizeFieldName(newLabel);
                ...
                <NameInput
                  ...
                  onCustomModeChange={(custom) => {
                    ...
                    if (!custom) {
                      ...
                      const autoGeneratedName = normalizeFieldName(tempField.label || '');
