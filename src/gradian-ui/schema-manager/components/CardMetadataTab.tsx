...
import { getSectionTranslatedTitle } from '../utils/schema-utils';
...
export function CardMetadataTab({ schema, onUpdate }: CardMetadataTabProps) {
  ...
  const handleDraftTitleChange = (value: string | Array<Record<string, string>>) => {
    if (!editingSectionDraft) return;
    if (isTranslationArray(value)) {
      const resolvedTitle = resolveFromTranslationsArray(value, defaultLang, defaultLang);
      const update: Partial<CardSection> = { titleTranslations: value, title: resolvedTitle };
      if (!editingCustomId) update.id = ensureKebabCase(resolvedTitle, 'card-section');
      updateDraftSection(update);
    } else {
      const update: Partial<CardSection> = { title: value, titleTranslations: undefined };
      if (!editingCustomId) update.id = ensureKebabCase(value, 'card-section');
      updateDraftSection(update);
    }
  };
                      ...
                      <div className="flex items-center justify-between gap-3 flex-wrap">
                        <div>
                          <CardTitle className="text-base">
                            {getSectionTranslatedTitle(section, language, labelUntitledSection)}
                          </CardTitle>
