...
import { getSchemaWithClientCache } from '../utils/client-schema-cache';
...
export function useSchemaById(
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Prefer client-side IndexedDB cache first; fallback to API (demo vs live handled by API route)
