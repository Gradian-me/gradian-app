...
const DynamicPageRenderer = dynamic(
  () => import('@/gradian-ui/data-display/components/DynamicPageRenderer').then(mod => ({ default: mod.DynamicPageRenderer })),
  { 
    ssr: false,
    loading: () => (
...
export default function PagesBuilderPage() {
  ...
  useEffect(() => {
    ...
    const loadSchema = async () => {
      try {
        const response = await fetch('/api/schemas/pages');
        ...
        console.error('Error loading pages schema:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const layoutProps = !mounted ? {} : !ENABLE_BUILDER
    ? { title: 'Access Denied', subtitle: 'The builder is disabled in this environment.', icon: 'OctagonMinus' as const }
    : { title: 'Page Builder', subtitle: 'Create and manage custom pages', icon: 'FileText' as const };
  useSetLayoutProps(layoutProps);
  ...
  if (!schema) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-gray-600">Please configure the pages schema first.</p>
      </div>
    );
  }

  return <DynamicPageRenderer schema={schema} entityName={schema.singular_name || 'Page'} />;
}
