...
import { useOptionsFromSchemaOrUrl } from "@/gradian-ui/form-builder/form-elements/hooks/useOptionsFromSchemaOrUrl";
...
export interface MultiSelectOption {
	...
	/** Color for badge (converted to style.badgeColor) */
	color?: string;
}
...
export interface MultiSelectProps
	...
	/** The current selected values (controlled mode) */
	value?: string[];
	...
	// Support for schemaId and sourceUrl like PopupPicker
	schemaId?: string;
	sourceUrl?: string;
...
const getIconComponentFromString = (icon?: string | React.ComponentType<{ className?: string }>): React.ComponentType<{ className?: string }> | undefined => {
...
// Helper to convert color to badge style
const getBadgeStyleFromColor = (color?: string): { badgeColor?: string } | undefined => {
	...
	// If it's a valid badge variant, return undefined (will use variant)
	const validVariants = ['default', 'secondary', 'destructive', 'success', 'warning', 'info', 'outline', 'gradient', 'muted'];
	if (validVariants.includes(color)) {
		return undefined;
	}
	
	// Otherwise, try to use as color
	return { badgeColor: color };
};
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
	(
		...
		const resolvedOptions = (schemaId || sourceUrl) ? fetchedOptions : providedOptions;

		// Convert and normalize options to MultiSelectOption format
		const normalizedOptions = React.useMemo((): MultiSelectOption[] | MultiSelectGroup[] => {
			if (!resolvedOptions || resolvedOptions.length === 0) {
				return [];
			}

			// If it's already normalized options from the hook
			if (schemaId || sourceUrl) {
				// Convert NormalizedOption[] to MultiSelectOption[] format
				return (resolvedOptions as NormalizedOption[]).map((opt) => {
					const iconComponent = opt.icon ? getIconComponentFromString(opt.icon) : undefined;
					const badgeStyle = getBadgeStyleFromColor(opt.color);
			...
			// At this point, resolvedOptions is MultiSelectOption[] | MultiSelectGroup[]
			const typedOptions = resolvedOptions as MultiSelectOption[] | MultiSelectGroup[];
			
			// If it's grouped options
			if (isGroupedOptions(typedOptions)) {
				return typedOptions.map(group => ({
					...group,
					options: group.options.map(opt => {
						const iconComponent = typeof opt.icon === 'string' ? getIconComponentFromString(opt.icon) : opt.icon;
						const badgeStyle = opt.color ? getBadgeStyleFromColor(opt.color) : opt.style;
						
						return {
							...opt,
							icon: iconComponent,
							...(badgeStyle && { style: { ...opt.style, ...badgeStyle } }),
						};
					}),
				}));
			}

			// If it's a flat array of options
			const flatOptions = normalizeOptionArray(resolvedOptions as any[]);
			return flatOptions.map((opt: any) => {
				const iconComponent = opt.icon ? getIconComponentFromString(opt.icon) : undefined;
				const badgeStyle = opt.color ? getBadgeStyleFromColor(opt.color) : undefined;
		...
		}, [resolvedOptions, schemaId, sourceUrl]);
		...
		const getAllOptions = React.useCallback((): MultiSelectOption[] => {
			...
			if (isGroupedOptions(normalizedOptions)) {
				allOptions = normalizedOptions.flatMap((group) => group.options);
			} else {
		...
		}, [normalizedOptions, deduplicateOptions]);
		...
		const filteredOptions = React.useMemo(() => {
			if (!searchable || !searchValue) return normalizedOptions;
		...
		}, [normalizedOptions, searchValue, searchable]);
									...
									<div
										...
										{selectedValues
											...
											.map((value) => {
												...
												// Determine badge variant from color if it's a valid variant
												const isValidBadgeVariant = (c?: string): boolean => {
													if (!c) return false;
													const validVariants = ['default', 'secondary', 'destructive', 'success', 'warning', 'info', 'outline', 'gradient', 'muted'];
													return validVariants.includes(c);
												};

												const badgeVariant = option.color && isValidBadgeVariant(option.color) 
													? (option.color as any)
													: undefined;
												...
												const badgeStyle: React.CSSProperties = {
													...
													// If color is a hex, use it
													...(option.color && option.color.startsWith('#') && !customStyle?.badgeColor && {
					...
					<PopoverContent
						...
						<Command>
							...
							<CommandList
								...
								{optionsError && (
									<CommandEmpty>Error: {optionsError}</CommandEmpty>
								)}
